...
def get_ds_config(ds_root, *, has_annots=False):
    ...
    if not cfg or IMAGE_KEY not in cfg:
        full_frame_dir = utils.prompt_folder_confirm(
            ds_root, DEFAULT_IMG_DIRS, IMAGE_KEY
        )
        cfg_update[IMAGE_KEY] = utils.path_to_relative_path(full_frame_dir, ds_root)

    if has_annots:
        if not cfg or ANNOTATION_KEY not in cfg:
            full_annot_dir = utils.prompt_folder_confirm(
                ds_root, DEFAULT_ANNOT_DIRS, ANNOTATION_KEY
            )
            cfg_update[ANNOTATION_KEY] = utils.path_to_relative_path(
                full_annot_dir, ds_root
            )
